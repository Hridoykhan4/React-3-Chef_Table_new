    ...
    <div className="card bg-base-100 shadow-sm">
      ...
      <div className="py-5 px-2 space-y-2">
        ...
        <div>
          ...
          <ul className="list-disc list-inside">
            {ingredients.map((element, i) => (
              <li className="text-gray-600 font-medium" key={i}>
                {element}
              </li>
            ))}
          </ul>
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex items-center text-gray-600 font-medium">
            <img className="w-6" src={calorieImg} alt="" />{" "}
        ...
        <div className="card-actions mt-5">
          <button
            disabled={recipe?.isDisabled}
            onClick={() => handleCook(recipe)}
            className="btn btn-success text-black rounded-l-full rounded-r-full px-5 py-5"
          >
            {recipe?.isDisabled ? "Already Ordered" : "Want To Cook"}
          </button>
        </div>
      </div>
    </div>
